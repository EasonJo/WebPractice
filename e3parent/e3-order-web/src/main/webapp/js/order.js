            ...
            $.ajax({
                ...
                success: function (msg) {
                    ...
                    if (msg.status != 0) {
                        ...
                        var mobile = '';
                        if (msg.address.mobile != 'undefined ') {
                            mobile = msg.address.mobile;
                        }
                        if (mobile == '' && msg.address.tel) {
                            mobile = msg.address.tel;
                        }
...
function giverCard() {
    ...
    $(".giverShow").live("mouseenter", function () {
        $(".giverItem").html(inputArea.val().replace(/\n/gi, "<br />")).show();
    ...
    $.post(web_url, {addr_id: addr_id}, function (msg) {
        ...
        if (msg.succes == 1) {
            ...
            $('#addrConfirm').css({
                ...
                'color': '#FFF',
                'cursor': 'pointer'
            }).removeClass('yhmStop').text('保存收货人信息');
            if (msg.status == 1) {
...
$(function () {
    $(".bankers a").hover(function () {
        $(".showbanker").removeClass("hide")
    }, function () {
        $(".showbanker").addClass("hide")
    })
    $(".spItem").each(function () {
        var goodsItem = $(this).find(".goodsItem").length;
        if (goodsItem > 3) {
            $(this).css({"height": "100px", "overflow-x": "hidden", "overflow-y": "auto"})
        }
    })
    /*支付及配送方式*/
...
function noGiverAddr() {
    ...
    if (!$("#edit_giver_addr_id").val()) {
        ...
        $("#addrGiverForm input").each(function () {
            $(this).val($(this).attr("def"))
        });
    }
    $("#addrGiverForm select").each(function () {
        $(this).val('')
    });
}
...
$(function () {
    ...
    $(".dmTip").hover(function () {
        $(".dmCont").show()
    }, function () {
        $(".dmCont").hide();
    })
    $(".dmCont").hover(function () {
        $(".dmCont").show();
    }, function () {
        $(".dmCont").hide();
    })
